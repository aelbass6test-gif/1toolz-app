...
const WalletPage: React.FC<WalletPageProps> = ({ wallet, setWallet, setSettings, orders, settings, treasury, setTreasury }) => {
  ...
  const cycleOrders = useMemo(() => {
    ...
    return orders.filter(o => {
      const isRelevantStatus = ['تم_توصيلها', 'تم_التوصيل', 'تم_التحصيل', 'مدفوعة', 'مدفوع', 'قيد_الشحن', 'تم_الارسال', 'مرتجع', 'فشل_التوصيل', 'مرتجع_جزئي', 'تمت_الاعادة_لشركة_الشحن', 'ملغي'].includes(o.status);
                                      ...
                                      {(() => {
                                          ...
                                          return (
                                              <span className={`inline-flex flex-row-reverse items-center justify-center gap-1.5 px-4 py-2 rounded-xl text-[11px] font-black border tracking-wide ${
                                                  isCancelledWithDeduction
                                                  ? 'bg-orange-50 text-orange-600 border-orange-200/50 dark:bg-orange-500/10 dark:text-orange-400 dark:border-orange-500/20'
                                                  : item.status === 'pending'
                                                  ...
                                                  <span className={`w-1.5 h-1.5 rounded-full ${
                                                      isCancelledWithDeduction ? 'bg-orange-500' :
